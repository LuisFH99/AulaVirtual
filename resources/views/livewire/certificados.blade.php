...
@section('content') --}}
    ...
    <div class="mx-4 row">
        <div class="col-md-6 p-3"><!--style="color: #2d529f" -->
        ...
        <div class="col-md-6 p-0 mt-4">
            <div class="card p-3 mt-5 mb-4">
                ...
                <select class="form-select form-select-sm" aria-label="Default select example" wire:model='curso_id'>
                    ...
                    @foreach ($cursos as $m)
                        <option value="{{$m->publicacion->curso->nombre}}">{{strtoupper($m->publicacion->curso->nombre)}}</option>
                            ...
                            @foreach ($certificado as $c)
                                <div class="mb-4">
                                    ...
                                    {{-- <div class="col-md-6 p-4"> --}}
                                        <a class="btn btn-primario btn-sm" href={{$c->ruta}} download="{{$c->ruta}}"> Descargar Certificado</a>
                                        <a class="btn btn-cancel btn-sm" href={{ asset($c->ruta) }} target="_blank">Visualizar</a>
                                    {{-- </div> --}}
                                    {{-- <button class="btn btn-primario btn-sm" wire:click.prevent="descargar ({{$c->id}})" >Descargar Certificado</button>
                                    <button class="btn btn-cancel btn-sm" wire:click.prevent="ver ({{$c->id}})" >Visualizar</button> --}}
                                </div>
                            @endforeach
                        </div>
                    @else
